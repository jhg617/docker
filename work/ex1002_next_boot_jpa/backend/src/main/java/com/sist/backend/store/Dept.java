...
// 위 어노테이션을 사용할때 Hibernate관련 필드를 직렬화에서 제외시킨다.
...
public class Dept {
    ...
    // 주 테이블(부모)의 외래키가 자식 엔티티(자식 테이블)에 존재 한다고 알고,
    // 자식 테이블의 외래키를 기준으로 관계를 매핑한다.
    // 즉, Dept 엔티티는 부모로서 Emp 엔티티의 dept라는 멤버변수를 기준으로
    // 관계를 정의한다.
    @OneToMany(mappedBy = "dept", cascade = CascadeType.ALL, fetch = FetchType.LAZY)
    @JsonIgnoreProperties({"dept"}) // Dept엔티티 기준으로 참조하는 Emp의 무한 참조를 막는다.
                                    //mappedBy에 명시한 값이 들어온다.
